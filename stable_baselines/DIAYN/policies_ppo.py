...
class DIAYNPPOPolicy(BasePolicy):
    """
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, scale=False, num_skills=None):
        ...
        super(DIAYNPPOPolicy, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=reuse,
                                             scale=scale, obs_phs=(obs_phs, processed_obs))
...
class FeedForwardPolicy(DIAYNPPOPolicy):
    """
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, layers=None, net_arch=None,
                 act_fun=tf.tanh, cnn_extractor=nature_cnn, feature_extraction="cnn", num_skills=None, **kwargs):
        self.Do = ob_space.shape[0]
        self.num_skills = num_skills
        super(FeedForwardPolicy, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=reuse,
                                                scale=(feature_extraction == "cnn"), num_skills=num_skills)
        ...
        with tf.variable_scope("model", reuse=reuse):
            ...
            (obs, self.z_one_hot) = self._split_obs(obs)
            obs = tf.layers.flatten(obs)
            with tf.variable_scope("discriminator", reuse=reuse):
                hidden = mlp(obs, layers, act_fun)
                self.logit = tf.layers.dense(hidden, self.num_skills, activation=None)
            self.intrinsic_reward = -1 * tf.nn.softmax_cross_entropy_with_logits_v2(self.z_one_hot, self.logit)
    ...
    def step(self, obs, state=None, mask=None, deterministic=False):
        if deterministic:
            action, value, neglogp, intrinsic_reward = self.sess.run([self.deterministic_action, self.value_flat, self.neglogp, self.intrinsic_reward],
                                                   {self.obs_ph: obs})
        else:
            action, value, neglogp, intrinsic_reward = self.sess.run([self.action, self.value_flat, self.neglogp, self.intrinsic_reward],
                                                   {self.obs_ph: obs})
        return action, value, intrinsic_reward, neglogp
