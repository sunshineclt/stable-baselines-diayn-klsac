...
class DIAYNPolicy(BasePolicy):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env=1, n_steps=1, n_batch=None,
                 num_skills=None, reuse=False, scale=False):
        assert len(ob_space.shape) == 1
        with tf.variable_scope("input", reuse=False):
            obs_phs = tf.placeholder(shape=(n_batch, ob_space.shape[0] + num_skills),
                                     dtype=tf.float32, name='Ob')
            processed_obs = tf.cast(obs_phs, tf.float32)
        super(DIAYNPolicy, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=reuse, scale=scale,
                                          obs_phs=(obs_phs, processed_obs))
...
class FeedForwardPolicy(DIAYNPolicy):
    """
    ...
    def __init__(self, sess, ob_space, ac_space, n_env=1, n_steps=1, n_batch=None, reuse=False, layers=None,
                 cnn_extractor=nature_cnn, feature_extraction="cnn", reg_weight=0.0, num_skills=None,
                 layer_norm=False, act_fun=tf.nn.relu, **kwargs):
        self.Do = ob_space.shape[0]
        self.Da = ac_space.shape[0]
    ...
    def _split_obs(self, obs):
        return tf.split(obs, [self.Do, self.num_skills], axis=1)

    def make_discriminator(self, obs=None, reuse=False, scope="discriminator"):
        if obs == None:
            obs = self.processed_obs

        (obs, z_one_hot) = self._split_obs(obs)
        obs = tf.layers.flatten(obs)

        with tf.variable_scope(scope, reuse=reuse):
            hidden = mlp(obs, self.layers, self.activ_fn, layer_norm=self.layer_norm)
            self.logit = tf.layers.dense(hidden, self.num_skills, activation=None)
